...
include_once('../google_auth/gpConfig.php');
...
$gClient->setApplicationName("Getting YouTube Data...");
...
$tokenSessionKey = 'token-' . $gClient->prepareScopes();
if (isset($_GET['code'])) {
    if (strval($_SESSION['state']) !== strval($_GET['state'])) {
        die('The session state did not match.');
    }
    $gClient->authenticate($_GET['code']);
    $_SESSION[$tokenSessionKey] = $gClient->getAccessToken();
    ...
    function getChannelVideos($channel_id) {
        ...
        foreach ($channels as $channel) {
            ...
            foreach($videos as $video) {
                embed_videos[$count] = generateEmbedLink($video->getSnippet()->getResourceId()->getVideoId(), 560, 315);
